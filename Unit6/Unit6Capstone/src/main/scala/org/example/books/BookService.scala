...
class BookService(bookCatalogPath: String) {
  ...
  private def checkBookIsTaken(book: Book): Either[String, User] =
    findBookLoan(book) match {
      ...
      case None => Left(s"Book ${book.id} is out on loan")
    }
  ...
  private def containsCaseInsensitive(text: String, substring: String): Boolean =
    text.toLowerCase.contains(substring.toLowerCase)

  private def outcomeMsg[T](res: Either[String, T]): String = res.left.getOrElse("OK")
  ...
  private def checkReserveLimits(user: User): Either[String, User] =
    ...
    else Left(s"You cannot loan more than $loanLimit books at the same time")
  ...
  private def updateBookLoans(f: Set[BookLoan] => Set[BookLoan]): Unit = synchronized( { bookLoans = f(bookLoans) } )
