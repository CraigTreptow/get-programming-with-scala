...
class BookParser(filePath: String) {
  ...
  val books: List[Book] = {
    loadCSVFile(filePath).flatMap { rowData =>
      Book.parse(rowData) match {
        ...
        case Failure(ex) =>
          logger.warn("Skipping book: Unable to parse row because " + s"of ${ex.getMessage} - row was $rowData")
          None
      }
    }
  }
  ...
  private def loadCSVFile(path: String): List[Map[String, String]] = {
    ...
    logger.info(s"Completed processing of file $path! " + s"${data.size} records loaded")
    data
  }
}
